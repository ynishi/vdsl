--- Decoder: reconstruct vdsl information from a ComfyUI prompt table.
...
--
-- Usage:
--   local decoder = require("vdsl.compilers.comfyui.decoder")
--   local info = decoder.decode(comfy_prompt)
...
local function unwind_conditioning(prompt, ref)
  ...
  elseif node.class_type == "ControlNetApplyAdvanced" then
    -- Advanced variant has separate positive/negative outputs;
    -- use the ref slot to determine which input to trace.
    if ref[2] == 1 then
      return unwind_conditioning(prompt, node.inputs.negative)
    else
      return unwind_conditioning(prompt, node.inputs.positive)
    end
  end
...
local function decode_casts(prompt, idx, ksampler_node)
  ...
  -- Global negative (from opts.negative) is extra entries at the end
...
local function decode_stage(prompt, idx)
  ...
  -- Also handle ControlNetApplyAdvanced (same inputs for decode purposes)
  for _, e in ipairs(find_nodes(idx, "ControlNetApplyAdvanced")) do
    cn_entries[#cn_entries + 1] = e
  end
...
local function decode_post_pixel(prompt, idx)
  ...
  local decoders = {
    ...
    ColorCorrect = function(node)
      -- Reverse of engine's mul_to_offset: offset → multiplier
      -- Node API: offset (-100..100, 0 = no change)
      -- VDSL API: multiplier (1.0 = no change, 1.1 = +10%)
      local function offset_to_mul(v)
        if v == nil or v == 0 then return 1.0 end
        return 1.0 + v / 100
      end
      return {
        type   = "color",
        params = {
          brightness = offset_to_mul(node.inputs.brightness),
          contrast   = offset_to_mul(node.inputs.contrast),
          saturation = offset_to_mul(node.inputs.saturation),
