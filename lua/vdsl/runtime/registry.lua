...
--- Set a custom registry backend.
-- Backend interface: { fetch_object_info(url, headers) -> table }
-- @param backend table or nil to reset to default (transport-based)
function Registry.set_backend(backend)
  if backend ~= nil and type(backend) ~= "table" then
    error("Registry.set_backend: backend must be a table", 2)
  end
  _backend = backend
end
...
function Registry.connect(url, opts)
  ...
  local info
  if _backend and _backend.fetch_object_info then
    info = _backend.fetch_object_info(url, headers)
  else
...
--- Resolve an image reference: upload local file or return server name as-is.
-- If the path contains '/' or '\' and the file exists locally, uploads it.
-- Otherwise assumes it's already a server-side filename.
-- @param image string local filepath or server filename
-- @return string server-side filename for use in LoadImage nodes
function Registry:resolve_image(image)
  if not self._url then
    error("cannot resolve_image without server URL (use connect())", 2)
  end
  if type(image) ~= "string" or image == "" then
    error("resolve_image: image is required", 2)
  end

  -- Check if it looks like a local path (contains path separator)
  if image:find("/") or image:find("\\") then
    if fs.exists(image) then
      local resp = self:upload(image)
      return resp.name
    end
  end

  -- Already a server-side filename
  return image
end

--- Upload a file to ComfyUI /upload/image endpoint.
-- @param filepath string local path of the file to upload
-- @param opts table|nil { subfolder = "...", overwrite = true }
-- @return table server response { name, subfolder, type }
function Registry:upload(filepath, opts)
  if not self._url then
    error("cannot upload without server URL (use connect())", 2)
  end
  if type(filepath) ~= "string" or filepath == "" then
    error("upload: filepath is required", 2)
  end
  opts = opts or {}
  local form_fields = {
    subfolder = opts.subfolder or "",
    type      = opts.type or "input",
    overwrite = opts.overwrite ~= false and "true" or "false",
  }
...
--- Upload a directory of files to ComfyUI.
-- Uploads all files in the given directory to a subfolder under input/.
-- @param dir string local directory path
-- @param subfolder string target subfolder on server (e.g. "training_satsuki/10_satsuki")
-- @param opts table|nil { pattern = ".*", overwrite = true }
-- @return table { uploaded = count, files = { ... } }
function Registry:upload_dir(dir, subfolder, opts)
  if not self._url then
    error("cannot upload_dir without server URL (use connect())", 2)
  end
  opts = opts or {}
  ...
  for _, filename in ipairs(entries) do
    if filename:match(pattern) then
      ...
      local resp = self:upload(filepath, {
        subfolder = subfolder,
        overwrite = opts.overwrite,
      })
      uploaded[#uploaded + 1] = { file = filename, resp = resp }
    end
  end

  return { uploaded = #uploaded, files = uploaded }
end
...
function Registry:run(render_opts, run_opts)
  ...
  local json_enc     = require("vdsl.util.json")
