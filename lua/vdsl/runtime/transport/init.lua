...
local function get_backend()
  ...
  _backend = require("vdsl.runtime.transport.curl")
  return _backend
end
...
function M.upload(url, filepath, form_fields, headers)
  check_url(url, "transport.upload")
  if type(filepath) ~= "string" or filepath == "" then
    error("transport.upload: filepath must be a non-empty string", 2)
  end
  return get_backend().upload(url, filepath, form_fields, headers)
end
