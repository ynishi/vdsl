...
-- @param form_fields table|nil extra form fields { key = value, ... }
-- @param headers table|nil optional HTTP headers
-- @return table parsed JSON response
function M.upload(url, filepath, form_fields, headers)
  local h = curl_headers(headers)
  local parts = { string.format(
    'curl -sf %s -X POST -F %s %s',
    CURL_TIMEOUT, shell_quote("image=@" .. filepath), h
  ) }

  if form_fields then
    for k, v in pairs(form_fields) do
      -- Insert -F flags before the header flags
      parts[1] = parts[1] .. " -F " .. shell_quote(k .. "=" .. tostring(v))
    end
  end

  parts[1] = parts[1] .. " " .. shell_quote(url)

  local body, ok = popen_read(parts[1])
  if ok and body and #body > 0 then
    return json.decode(body)
  end
  error("failed to upload " .. filepath, 2)
end
