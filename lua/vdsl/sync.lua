--- Sync: 3-location file synchronization engine (Domain layer).
-- Pure business logic: state management, registration, duplicate detection.
-- No direct infrastructure access — all I/O through injected dependencies:
--   db      — runtime/db (SQLite operations)
--   backend — runtime/sync (file transfer + content hash)
--
...
-- @param backend table  runtime/sync module (push/pull/list/exists/hash)
-- @param opts table|nil  { pod_id, ssh_key, bucket }
-- @return Sync
function Sync.new(db, backend, opts)
  if not db then
    error("Sync.new: db is required", 2)
  end
  if not backend then
    error("Sync.new: backend is required", 2)
  end
  opts = opts or {}
...
-- via backend.hash() (pluggable: pure Lua default, Rust for perf).
...
function Sync:register(file_path, file_type, opts)
  ...
  if not file_hash and file_type == Sync.TYPE_IMAGE and file_path:match("%.png$") then
    if fs.exists(file_path) then
      file_hash = self.backend.hash(file_path)
    end
  end
...
function Sync:push_file(file_path, dest_loc, dest_path)
  ...
  local ok, err = self.backend.push(file_path, dest_loc, dest_path, self:_opts())
  if ok then
...
function Sync:pull_file(src_loc, src_path, dest_path, reg_opts)
  ...
  local ok, err = self.backend.pull(src_loc, src_path, dest_path, self:_opts())
...
-- @param opts table|nil  { pattern = "*.png" }
...
function Sync:scan(dir, file_type, opts)
  ...
  for _, fpath in ipairs(files) do
    ...
    local fsize = fs.file_size(fpath)
    if existing then
...
function Sync:register_generation(gen)
  ...
  if gen.output and fs.exists(gen.output) then
    local fsize = fs.file_size(gen.output)
